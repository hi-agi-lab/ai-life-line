...
真实场景验收集上最有意思的发现：**外观距离与语义距离是互补的，不是冗余的**。
...
- **湿沥青上的天空反光 vs 浅积水，单帧下光学/语义同构**——物理规则、
  开放词汇、2B/4B/9B VLM 我们全试过，全被骗（AI-Reference 单独判读湿沥青
...
我们认为把失败分析跟全绿数字一起发布，才是负责任的发布该有的样子。
...
本项目是商用软件。真实场景验收集
**不随仓库分发**；`acceptance_manifest_v2.json` 记录全部测试用例，复现需按
清单本地采集图片。
