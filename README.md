...
<img src="images/hero_banner.jpg" alt="城市生命线平台 首屏" width="100%"/>
...
城市拥有成千上万的路由摄像头，但一个井盖丢了、一段路淹了、一排灯灭了，
往往还要等市民打电话投诉。**城市生命线平台** 把存量摄像头变成三类民生
...
| **AI-Detect-Uni**（免提示提议 + 区域嵌入） | L2 语义 | "这块 ROI 相对登记基线**语义**上变了吗？" |
...
- **湿沥青的天空反射光膜 vs 浅积水，单帧下光学/语义同构**——物理规则、
  开放词汇、2B VLM 我们全试过，全被骗（Ref 单独判读湿沥青负样本
  P=0.647）。区分信息不在这一帧里，而在时间维度：登记干态基线
...
代码 license 待定（见 PITCH.md §4）。真实图集**不随仓库分发**；
`acceptance_manifest_v2.json` 记录全部 case，复现需按清单本地采集图片。
